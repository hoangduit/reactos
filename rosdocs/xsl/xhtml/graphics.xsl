...
<xsl:template name="process.image">
  ...
  <xsl:variable name="viewport">
    <xsl:choose>
      <xsl:when test="inlinegraphic                       | ancestor::inlinemediaobject                       | ancestor::inlineequation">0</xsl:when>
      <xsl:otherwise>
...
-->
  ...
  <xsl:variable name="img">
    <xsl:choose>
      <xsl:when test="@format = 'SVG'">
        <object xmlns="http://www.w3.org/1999/xhtml" data="{$filename}" type="image/svg+xml">
  ...
  <xsl:choose>
    <xsl:when test="$viewport != 0">
      <table xmlns="http://www.w3.org/1999/xhtml" border="0" summary="manufactured viewport for HTML img" cellspacing="0" cellpadding="0">
